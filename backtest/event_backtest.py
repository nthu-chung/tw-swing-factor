...
def _backtest_portfolio(symbols: Optional[List[str]] = None,
    ...
    if policy_enabled:
        ...
        #
        # 選股規則**不在這裡**。綜合分數門檻與趨勢硬門檻是 legacy 九因子策略
        # 自己的看法，2026-08-17 已搬到 `factor_engine.legacy_factors`。引擎只
        # 強制市場強制的事（T+1、漲跌停、處置、整股、現金）——「MA20 要不要在
        # MA60 之上」不是市場規定，是策略意見，不該由引擎替所有路徑決定。
        sig = factors.legacy_selection(
            panel,
            min_composite=config.MIN_COMPOSITE,
            trend_guard=config.TREND_GUARD_ENABLED,
        )
        picks_by_date = {}
