...
⚠ **research-only(原始快取事件研究)**:本腳本直讀 `_cache` 的原始價格,繞過
`backtest` 的還原價與完整性 fail-closed 閘門,也不經過事件驅動引擎。它回答的是
「這個方向值不值得做成策略」,**不是**投組績效,不得升格成正式策略入口。要出正式
數字必須改走 `universes.historical_pit_universe()` + `backtest.backtest_portfolio()`。
