...
>
> **證券別污染（2026-08-15 發現並修復，全表既有數字一律需重驗）**：
...
> 創新板 28／DR 11）；PIT 逐日快照混進 28 檔創新板 + 4 檔 DR，
> `outputs/universe_top100.json` 也含 1 檔創新板。興櫃沒有 ±10% 漲跌停
> （2026-05 單日 |ret|>10.5% 佔比為興櫃 3.872% vs 上市 0.034%，最大 +57.17%），
> 而動能因子找的正是那種標的——偏誤方向是**系統性灌高 Sharpe**。
> 修復後候選池組成會改變，因此**本表所有既有績效數字都是在不同的池上算的**，
> 在合格重跑前一律視為需重驗；不得因為這次修正而宣稱任何策略變好或變壞。
...
7. 策略只負責訊號與排名；**持有、退出、資金槽與曝險屬於 `StrategyPositionPolicy`**
   （`strategies/position_policy.py`），成交限制屬於事件引擎。登記新策略時要分清楚
   哪些參數是訊號、哪些是 policy —— 兩者的失效方式不同，也各自有自己的 rules hash。
   責任邊界見 `ARCHITECTURE.md`，行為規格見 `STRATEGY_POSITION_POLICY_SPEC.md`。
   **這一層是可稽核性的基礎設施，本身不是任何策略的證據；沒有策略因為它而升級
   證據等級，本檔所有數字也都沒有重跑。**
...
| holdout 使用紀錄（2026-08-15） | 新增 append-only 的揭露台帳 `evaluation/holdout.py` → `outputs/holdout_ledger.jsonl`（strategy hash／OS 起訖／reveal time／git commit；雜湊鏈防靜默改寫）。**S19 的既有 OS 在 `KNOWN_CONSUMED_HOLDOUTS` 裡明確登記為 `consumed_pseudo_oos`（已看過的資料窗 2024-06-23~2026-08-06，當時的 OS 段 2025-11-19~2026-06-18），任何重跑都會被標 `holdout_previously_seen=True`，不得重設為 clean。** 之所以寫成程式常數而不是台帳裡的一列：`outputs/` 不進版控，狀態只存在某台機器的 jsonl 的話，換一台 clone 就變回 clean。**這是紀錄機制，不是新證據：S19 沒有重跑任何數字，證據等級維持 blocked。** |
