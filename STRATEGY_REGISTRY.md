...
| S19 | 籌碼確認的風險調整動能（CRM） | `active hypothesis` | **首個在自建還原價上、IS 五相位 Sharpe 全 >1 的策略**（中位 1.64／最小 1.07）；OS 中位 3.80 但僅 12~15 筆。IS 勝無成本等權基準（1.64 vs 1.17）、OS 敗（3.80 vs 4.20）。 |
...
| 資料與 universe | **自建還原價**（`price_adjust.py`，除權息回溯）；top300 候選池排除 17 檔殘留斷點股後為 283 檔；每日動態 top100。 |
| 進出場 | 10 檔等權、**20 日**再平衡、MA60 出場（次日開盤）、**-15%** 硬停損、T+1 開盤、含手續費與證交稅。 |
| 證據等級 | IS 五相位 Sharpe 全部 >1（中位 1.644／最小 1.070／最大 2.014）；OS 五相位中位 3.802／最小 3.169。**首個在非 bypass 價格上達成的結果**。 |
| 關鍵機制 | 弱訊號（IC≈0.04）下**周轉率才是主導**。同訊號在 5日再平衡+MA20+-8% 下 IS 中位僅 0.93 且兩個相位為負；降周轉後交易數 184→57，五相位全部轉正。16 格掃描中所有 MA60 配置皆在前段，方向一致。 |
| 已知失效／偏誤 | OS 僅 12~15 筆無統計意義；勝率 29~40% 靠賺賠比 5.7~7.3；OS 段普漲下無成本等權基準 Sharpe 4.20 **高於本策略**；候選池倖存者未消；參數在 IS 上選過（以相位中位數為準則）。 |
| 下一個可證偽測試 | `freeze_manifest.py` 凍結 → `forward_test.py` 做 snapshot 後 forward-only；擴到 survivorship-free PIT 全池後看 IS 增量是否存活。 |
